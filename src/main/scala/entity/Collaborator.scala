...
class Collaborator(val login: String,
  ...
  lazy val owner = User.withLogin(ownerLogin)

  lazy val repo = owner match {
...
object Collaborator {
  ...
  def collaborator(user: User) = {
    DAO.select("SELECT user_login, owner_login, repo_name FROM collaborators WHERE user_login = ?", user.login) {
        row => new Collaborator(row.getString("user_login"), row.getString("owner_login"), row.getString("repo_name"))
      }
  }
